...
class Input:
    ...
    def __str__(self) -> str:  # pragma: no cover
        return (
            ...
            + "\n"
            + "raw_output_unit: "
            + self.raw_output_unit
            + "\n"
            + "decimals: "
            + str(self.decimals)
            + "\n"
            + "unitless_output: "
            + str(self.unitless_output)
        )
    ...
    def __filter_decimals(self, command: str) -> int:
        if "round" in command:
            ...
            if command_split[3] == "round":
                round_value: str = command_split[
                    4
                ]  # When the input value and unit are combined and no int output is specified before the round parameter
            elif command_split[4] == "round":
                round_value: str = command_split[
                    5
                ]  # When the input value and unit are not combined or int output is specified before the round parameter
            elif command_split[5] == "round":
                round_value: str = command_split[
                    6
                ]  # When the input value and unit are not combined and int output is specified before the round parameter
            else:
                raise InvalidNumberOfInputArgumentsError(command_split)
