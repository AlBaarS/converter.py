...
class Orchestrator:
    ...
    def __which_unit(self, unit: str) -> dict[str, str]:
        out_unit: dict[str, str] = {"unit": "", "type": ""}
        for found_unit in [
    ...
    def orchestrate_conversion(self, command: str) -> str:
        ...
        if input_unit_and_type["type"] != output_unit_and_type["type"]:
            raise IncompatibleUnitsError(
                f"{input_unit_and_type["unit"]} ({input_data.get_raw_input_unit()})",
                f"{output_unit_and_type["unit"]} ({input_data.get_raw_output_unit()})",
            )
        else:
        ...
        for ConverterClass in [
            ...
            if ConverterClass().get_type() == unit_type:
                ...
                if input_data.get_unitless_output():
                    ...
                    output: str = (
                        f"{str(self.__round(output_value, input_data.get_decimals()))} {output_unit.replace("_", " ")} ({output_symbol})"
                    )
        if output != "":
            return output
        else:
